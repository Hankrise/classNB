...
#' M-step of expectation maximization with closed-form solution
...
#' Perform inference of HMM negative binomial model using closed-form approach
#' @param X a vector of observations.
#' @param nbS a integer specifying the numbers of states.
...
#' @examples
#' data(toyexample)
#' # Perform inference of mixture of negative binomial models using closed-form approach
#' resHMM_CF <- HMM_CF(X = toydata, nbS = 3)
#' @seealso \code{\link{HMM_NS}}, \code{\link{HMM_NS}}
HMM_CF <- function(X, nbS, viterbi=TRUE,iterMax=500, threshold=1e-7){
