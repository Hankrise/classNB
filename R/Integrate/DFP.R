...
DFP = function(objfun, gradfun, x_initial, eps = 1.0e-6){
  ite = 0
  ...
  while(ite < 10000){
    ...
    if (as.numeric(t(s)%*%delta_g)==0 || as.numeric(t(delta_g)%*%D%*%delta_g)==0){
      break
    }
    D = D + (s%*%t(s))/as.numeric(t(s)%*%delta_g) - (D%*%delta_g%*%t(delta_g)%*%D)/as.numeric((t(delta_g)%*%D%*%delta_g))
    #delta_x = x_last - x
